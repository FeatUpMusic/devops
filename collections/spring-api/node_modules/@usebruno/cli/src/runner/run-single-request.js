...
const fs = require('fs');
const { each, extend, get } = require('lodash');
...
const runSingleRequest = async function (
  ...
  try {
    ...
    try {
      ...
      // If --bail has signalled a halt, abort this request before it goes out.
      if (bailAbortSignal) {
        request.signal = bailAbortSignal;
      }
      ...
      if (err?.response) {
        ...
        // Detect bail-driven abort: don't surface this as a request "error" — it's a bail-skip.
        const wasAborted = err?.code === 'ERR_CANCELED'
          || err?.name === 'CanceledError'
          || err?.name === 'AbortError'
          || bailAbortSignal?.aborted;
        if (wasAborted) {
          return {
            ...
            response: { status: 'skipped', statusText: 'bail-halted', data: null, responseTime: 0, duration: 0, size: 0 },
            error: null,
            status: 'skipped',
            skipped: true,
            skipReason: 'bail',
